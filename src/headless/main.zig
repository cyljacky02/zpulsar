...
//! issue #26's Hostname Attribution. Brings the `zPulsarNet` session up, runs
//! the consumer, Engine, and reverse-lookup threads, and renders a live
//! per-process In-session Totals table from acquired Snapshots — real image
//! names, dimmed "(exited)" rows, each row's Flows beneath it with Lingering
//! ones dimmed and remote names marked by the tier that produced them, health
//! flags. Requires elevation.
//!
