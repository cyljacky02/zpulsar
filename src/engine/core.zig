...
pub const Core = struct {
    ...
    pub fn applyEvent(
        ...
        switch (ev.op) {
            .send, .recv => {
                ...
                const live = try self.flows.touch(self.gpa, flows.flowKey(ev), idx, now_ms, &self.names);
    ...
    /// Time-driven Flow maintenance: Linger expiry, UDP age-outs, and the
    /// Hostname Attribution upkeep — reverse-lookup results in, candidates
    /// out, idle cache entries evicted. Called at the Engine's flush-tick
    /// cadence.
